...
public class UserGetController : ControllerBase
{
    ...
    string connect = "Server=localhost;port=64754;Database=Click;Uid=root;pwd=root;charset=utf8";
    ...
    public async Task<IActionResult> GetUserId(int id)
    {
         var mysqlConnect = new MySqlConnection(connect);
        await mysqlConnect.OpenAsync();
        var commandName = "SELECT name FROM Click WHERE id = @Id";
        var commandMail = "SELECT mail FROM Click WHERE id = @Id";
        var commandPass = "SELECT pass FROM Click WHERE id = @Id";
        var commandReplace_pass = "SELECT replace_pass FROM Click WHERE id = @Id";
        var commandBalans = "SELECT balans FROM Click WHERE id = @Id";
        ...
        mysqlCommandMail.Parameters.Add("@Id", MySqlDbType.Int64).Value = id;
        mysqlCommandReplace_pass.Parameters.Add("@Id", MySqlDbType.Int64).Value = id;
        mysqlCommandBalans.Parameters.Add("@Id", MySqlDbType.Int64).Value = id;
        mysqlCommandPass.Parameters.Add("@Id", MySqlDbType.Int64).Value = id;
                                ...
                                foreach (DataRow dataRowReplace_pass in dataSetReplace_pass.Tables[0].Rows)
                                {
                                    for (int f = 0; f < dataSetReplace_pass.Tables[0].Columns.Count; f++)
                                    {
                                        foreach (DataRow dataRowBalans in dataSetBalans.Tables[0].Rows)
                                        {
                                            for (int d = 0; d < dataSetBalans.Tables[0].Columns.Count; d++)
                                            {
                                                var user = new User(dataRowName[i].ToString(),dataRowMail[j].ToString(),dataRowPass[g].ToString(),dataRowReplace_pass[f].ToString(),dataRowBalans[i].ToString());
                                                listUser.Add(user);
                                            }
                                        }
                                    }
